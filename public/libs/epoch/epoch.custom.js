      ...
      for (var l, y, n, q, p, m = arguments, k = m[0], z = 0; z < m.length; z++) {
        ...
        if ("object" == typeof x) {
          ...
          if (k = x.replace(a, function (a, l, f, y, k, w) {
            ...
            if (f)
              if ("$" == y && (y = m[++z]), "#" == f) p.id = y;
              else if (l = (a = p.className) &&
              (" " + a + " ").replace(" " + y + " ", " "), "." == f) p.className = a ? (l + y).substring(1) : y;
            else if ("!" == x) {
              var A;
...
(function () {
  ...
  Epoch.Util.defaults = function (c, a) {
    var h, v;
    v = Epoch.Util.copy(c);
    for (h in a) null != c[h] && null != a[h] ? !Epoch.isArray(c[h]) && Epoch.isObject(c[h]) &&
      Epoch.isObject(a[h]) ? v[h] = Epoch.Util.defaults(c[h], a[h]) : v[h] = c[h] : v[h] = null != c[h] ? c[h] : a[h];
    return v
  };
  Epoch.Util.formatSI = function (c, a, h) {
    ...
    d = "KMGTPEZY".split("");
    for (u in d)
      if (s = d[u], v = Math.pow(10, 3 * ((u | 0) + 1)), c >= v && c < Math.pow(10, 3 * ((u | 0) + 2))) {
        c /= v;
        if ((c | 0) !== c || h) c = c.toFixed(a);
        return "" + c + " " + s
      }
  ...
  Epoch.Util.formatBytes = function (c, a, h) {
    ...
    for (u in e)
      if (d = e[u], v = Math.pow(1024, (u | 0) + 1), c >= v && c < Math.pow(1024, (u | 0) + 2)) {
        c /= v;
        if ((c | 0) !== c || h) c = c.toFixed(a);
        return "" + c + " " + d
      }
  ...
  Epoch.Util.domain = function (c, a) {
    ...
    for (l = c.length; d < l; d++)
      for (v = c[d], n = v.values, e = 0, f = n.length; e < f; e++) v = n[e], null == u[v[a]] && (h.push(v[a]), u[v[a]] = !0);
    return h
  };
    ...
    c.prototype.trigger = function (a) {
      ...
      if (null != this._events[a]) {
        h = function () {
          ...
          h = [];
          d = a = 1;
          for (c = arguments.length; 1 <=
            c ? a < c : a > c; d = 1 <= c ? ++a : --a) h.push(arguments[d]);
          return h
  ...
  Epoch.Chart.Base = function (c) {
    ...
    a.prototype.setData = function (a) {
      ...
      e = 0;
      for (f = a.length; e <
        f; e++) d = a[e], h = ["layer"], h.push("category" + c), d.category = c, null != d.label && h.push(Epoch.Util.dasherize(d.label)), d.className = h.join(" "), c++;
      return this.data = a
    };
    ...
    a.prototype.extent2 = function (a) {
      return [0, d3.max(this.data, function (c) {
        return d3.max(c.values, a)
      })]
    };
    return a
  ...
  Epoch.Chart.Canvas = function (c) {
    function a(c) {
      ...
      this.pixelRatio = null != this.options.pixelRatio ? this.options.pixelRatio :
        null != window.devicePixelRatio ? window.devicePixelRatio : 1;
      this.canvas.css({
        width: "" + this.width + "px",
  ...
  k = function () {
    ...
    c.getStyles = function (a, h) {
      ...
      d = [];
      q = 0;
      for (p = f.length; q < p; q++) l = f[q], n = l.tagName.toLowerCase(), null != l.id && 0 < l.id.length && (n += "#" + l.id), null != l.className &&
        0 < l.className.length && (n += "." + jQuery.trim(l.className).replace(/\s+/g, ".")), d.push(n);
      d.push("svg");
...
(function () {
  ...
  Epoch.Chart.Plot = function (f) {
    function d(c) {
      ...
      h = 0;
      for (v = u.length; h < v; h++) a = u[h], this.margins[a] =
        this.options.margins[a], null != c[a] || this.hasAxis(a) || (this.margins[a] = 6);
    ...
    d.prototype.y = function () {
      return d3.scale.linear().domain(this.extent2(function (c) {
        return c.y
      })).range([this.innerHeight(), 0])
    };
    d.prototype.bottomAxis =
      function () {
...
(function () {
  ...
  Epoch.Chart.Area = function (d) {
    ...
    e.prototype.y = function () {
      ...
      e = this.data;
      d = 0;
      for (u = e.length; d < u; d++)
        for (a in h = e[d], f = h.values, f) h = f[a], null != c[a] && (c[a] += h.y), null == c[a] && (c[a] = h.y);
  ...
  Epoch.Chart.Bar = function (f) {
    ...
    d.prototype.y = function () {
      ...
      return "horizontal" === this.options.orientation ? (a = this.extent(function (a) {
        return Math.max(parseFloat(a.y) + parseFloat(a.c), a.h);
    ...
    d.prototype._remapData = function () {
      ...
      p = this.data;
      t = 0;
      for (y = p.length; t < y; t++)
        for (e = p[t], a = "bar " + e.className.replace(/\s*layer\s*/, ""), k = e.values, l = 0, n = k.length; l < n; l++) c = k[l], null == f[q = c.x] && (f[q] = []), f[c.x].push({
          label: e.category,
          y: c.y,
          c: c.c,
          l: c.l,
    ...
    d.prototype._drawHorizontal = function () {
      ...
      d.transition().duration(600).attr("x", function (a) {
        ...
        $(this.childNodes).html(a.y);
        return a.y;
      });

      //confidence interval
      d2 = a.selectAll("polyline").data(function (a) {
        return a.values
      });
      d2.transition().duration(600).attr("x", function (a) {
        return f(a.label)
      }).attr("y", function (a) {
        return t(a.y);
      }).attr("points", function (a) {
        if(a.l && a.h) {
          return create_points(f(a.label) + (f.rangeBand() / 2), t(a.y), f.rangeBand(), c - t(a.c), t(a.l), t(a.h));
        } else {
          return "";
        }
        //  return create_points(f(a.label) + (f.rangeBand() / 2), t(a.y), f.rangeBand(), c - t(a.c), 0, 0);
      }).attr("width", "120").attr("height", function (a) {
        return t(a.l) - t(a.h);
      }).attr("foo", function (a) {
        $(this.childNodes).html(a.c);
        return a.c;
      });

      d2.enter().append("rect").attr("class", function (a) {
        return a.className
      }).attr("x", function (a) {
        return f(a.label)
      }).attr("y",
        ...
        return c - t(a.y)
      }).append("title").html(function (a) {
        return a.y;
      }).attr("class", "esesc_bar_tooltip");

      //confidence interval
      d.enter().append("polyline").attr("class", function (a) {
        return "epoch_bar_ci"
      }).attr("x", function (a) {
        return f(a.label)
      }).attr("y", function (a) {
        ...
        return a.c;
      }).attr("class", "esesc_ci_tooltip");

      var cnt = 0;
      this.svg.selectAll('text').attr('transform', function (el) {
        if(this.style['text-anchor'] == 'middle') {
          cnt ++;
          return 'rotate(-30,15,55)';
        }
        return '';
      });
      if(cnt > 0) {
        if(this.width / cnt > 100)
          this.svg.selectAll('text').attr('transform', '');
      }

      d2.exit().transition().duration(150).style("opacity", "0").remove();
...
(function () {
  ...
  Epoch.Chart.Scatter = function (f) {
    ...
    d.prototype.draw = function () {
      ...
      c.transition().duration(500).attr("class", function (a) {
        return "dot cat" + a.c;
      }).attr("points", function(a) {
        return create_shape(a.m, h(a.x), e(a.y));
      });
      ...
      c.enter().append("polygon").attr("class", function (a) {
        return "dot cat" + a.c;
      }).attr("points", function (a) {
        return create_shape(a.m, h(a.x), e(a.y));
      }).attr("onclick", function (a) {
        return "dot_redirect(" + a.i + ")";
      });
  ...
  Epoch.Time.Plot = function (d) {
    ...
    e.prototype._prepareTimeAxes = function () {
      ...
      for (d = f.length; c < d; c++)
        if (a = f[c], null != a.values && 0 < a.values.length) {
    ...
    e.prototype._updateTicks = function (a) {
      if (this.hasAxis("top") || this.hasAxis("bottom"))
        if (++this._tickTimer % this.options.ticks.time || this._pushTick(this.options.windowSize,
    ...
    e.prototype._updateTimeAxes = function () {
      ...
      if (this.hasAxis("top") || this.hasAxis("bottom")) {
        ...
        l = [];
        e = 0;
        for (f = t.length; e < f; e++) d = t[e], d.x += c, this.hasAxis("bottom") && d.bottomEl.attr("transform", "translate(" + (d.x + 1) + ",0)"), this.hasAxis("top") && d.topEl.attr("transform", "translate(" + (d.x + 1) + ",0)"), d.enter ? d.opacity +=
          a : d.exit && (d.opacity -= a), d.enter || d.exit ? (this.hasAxis("bottom") && d.bottomEl.css("opacity", d.opacity), this.hasAxis("top") ? l.push(d.topEl.css("opacity", d.opacity)) : l.push(void 0)) : l.push(void 0);
        return l
      }
    };
...
(function () {
  ...
  Epoch.Time.Area = function (d) {
    ...
    e.prototype.setStyles = function (c) {
      ...
      null != c.stroke && (this.ctx.strokeStyle =
        c.stroke);
    ...
    e.prototype._drawAreas = function (c) {
      ...
      for (d = k = n = this.data.length - 1; 0 >= n ? 0 >= k : 0 <= k; d = 0 >= n ? ++k : --k) {
        ...
        d = s[0];
        e = s[1];
        for (s = s[2]; - 2 <= --d && 0 <= --e;) a = f.values[e], a = [(d + 1) * t + c, l(a.y + a.y0)], s && (a[0] += t), d === this.options.windowSize -
          1 ? this.ctx.moveTo.apply(this.ctx, a) : this.ctx.lineTo.apply(this.ctx, a);
        d = s ? (d + 3) * t + c : (d + 2) * t + c;
  ...
  Epoch.Time.Bar = function (d) {
    ...
    e.prototype.setStyles = function (c) {
      ...
      this.ctx.strokeStyle =
        null == c.stroke || "none" === c.stroke ? "transparent" : c.stroke;
      if (null != c["stroke-width"]) return this.ctx.lineWidth = c["stroke-width"].replace("px", "")
    };
    e.prototype.draw = function (c) {
...
(function () {
  ...
  Epoch.Time.Heatmap = function (f) {
    ...
    d.prototype._prepareEntry = function (a) {
      ...
      k = a.histogram;
      for (d in k) a =
        k[d], e = parseInt(d / c), 0 > e ? e = 0 : e >= this.options.buckets && (e = this.options.buckets - 1), f.buckets[e] += a;
      d = a = 0;
...
(function () {
  ...
  Epoch.Time.Line = function (d) {
    ...
    e.prototype.draw = function (c) {
      ...
      p = [];
      y = 0;
      for (n = q.length; y < n; y++)
        if (f = q[y], 0 < f.values.length) {
          ...
          e = k[1];
          for (k = k[2]; - 2 <= --d &&
            0 <= --e;) a = f.values[e], a = [(d + 1) * m + c, l(a.y)], k && (a[0] += m), d === this.options.windowSize - 1 ? this.ctx.moveTo.apply(this.ctx, a) : this.ctx.lineTo.apply(this.ctx, a);
...
function create_shape (shape_id, x, y, scale) {
  var r;
  if(!scale)
    scale = 1;
  switch(shape_id) {
    case 0: r = 3 * scale; 
      return (x-r).toString()+","+(y-r).toString()+" "+(x+r).toString()+","+(y-r).toString()+" "+(x+r).toString()+","+(y+r).toString()+" "+(x-r).toString()+","+(y+r).toString();
      break;
    case 1: r = 4 * scale; 
      return (x-r).toString()+","+(y+r).toString()+" "+x.toString()+","+(y-r).toString()+" "+(x+r).toString()+","+(y+r).toString();
      break;
    case 2: r = 4 * scale; 
      return (x-r).toString()+","+y.toString()+" "+x.toString()+","+(y-r).toString()+" "+(x+r).toString()+","+y.toString()+" "+x.toString()+","+(y+r).toString();
      break;
    case 3: r = 3 * scale;
      return (x-r).toString()+","+(y+r).toString()+" "+x.toString()+","+(y-2*r).toString()+" "+(x+r).toString()+","+(y+r).toString()+" "+(x-1.5*r).toString()+","+(y-r).toString()+" "+(x+1.5*r).toString()+","+(y-r).toString();
      break;
    case 4: r = 2 * scale;
      return (x-r).toString()+","+(y+r).toString()+" "+(x-1.5*r).toString()+","+(y-r).toString()+" "+x.toString()+","+(y-2*r).toString()+" "+(x+1.5*r).toString()+","+(y-r).toString()+" "+(x+r).toString()+","+(y+r).toString();
      break;
    case 5: r = 2 * scale; 
      return (x-2*r).toString()+","+(y+1.5*r).toString()+" "+(x-r).toString()+","+(y-1.5*r).toString()+" "+(x+2*r).toString()+","+(y-1.5*r).toString()+" "+(x+r).toString()+","+(y+1.5*r).toString();
      break;
    default: return "";
  }
}

function create_points (x, y, w, c, min, max) {
  var points = '';
  points += (x - w/2).toString() + ',' + max.toString() + ' ';
  points += (x + w/2).toString() + ',' + max.toString() + ' ';
  points += x.toString() + ',' + max.toString() + ' ';
  points += x.toString() + ',' + min.toString() + ' ';
  points += (x - w/2).toString() + ',' + min.toString() + ' ';
  points += (x + w/2).toString() + ',' + min.toString();
  return points;
}
